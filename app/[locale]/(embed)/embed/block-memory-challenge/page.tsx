...
// Dynamically import the Block Memory Challenge game component (using named export)
const BlockMemoryChallengeGameComponent = dynamic(
  () => import('@/app/[locale]/(main)/games/block-memory-challenge/components/PatternRecallGame').then(mod => mod.PatternRecallGame),
  {
...
export default function BlockMemoryChallengeEmbedPage() {
  return (
    ...
    <div className={`flex items-center justify-center ${inter.className}`}>
      <div className="embedded-game-container p-2 w-full max-w-lg mx-auto">
        <BlockMemoryChallengeGameComponent />
      </div>
    </div>
  )
}
